...
async function downloadTikTok() {
    ...
    try {
        ...
        const apiUrl = `/api/download/tiktok-v2?apikey=${apiKey}&url=${encodeURIComponent(url)}`;
        const res = await fetch(apiUrl);
        const data = await res.json();

        if (data.status && data.result) {
            // Pilih video No Watermark / HD
            const videoUrl = data.result.data.hdplay || data.result.data.play || "";
            const audioUrl = data.result.data.music || "";
            ...
            resultBox.innerHTML = `
                <p><b>${data.result.data.title || "No Title"}</b> by <em>tiyarmybe</em></p>
